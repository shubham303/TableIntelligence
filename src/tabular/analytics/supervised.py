"""Supervised learning family — fast-lane gradient-boosted trees.

train_classifier / train_regressor build a scikit-learn Pipeline (shared
preprocessing + a HistGradientBoosting estimator), fit it on a proper train/test
split, and return a TrainedModel that bundles the fitted preprocessing so new
rows are transformed identically at predict time (no train/serve skew).

HistGradientBoosting is sklearn-native (no fragile system deps) and is a strong
default on tabular data. It is a drop-in swap point for xgboost / lightgbm /
autogluon (the slow lane) later.
"""
...
def train_classifier(store: Any, target: str) -> TrainedModel:
    """Train a gradient-boosted classifier on the table (fast lane)."""
    return _train(store, target, task="classification")


def train_regressor(store: Any, target: str) -> TrainedModel:
    """Train a gradient-boosted regressor on the table (fast lane)."""
    return _train(store, target, task="regression")


def _train(store: Any, target: str, task: str) -> TrainedModel:
    ...
    # Trees don't need scaling; still impute + one-hot for uniform handling.
...
def evaluate(store: Any, model: TrainedModel) -> Result:
    ...
    return Result(
        ...
        summary=summary,
        values=values,
        metadata={"target": model._target, "task": model._task, "n_test": int(len(y_test))},
    )
