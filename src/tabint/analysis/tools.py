...
structure (relationships/join/sql/create_table/insert_into), scratchpad,
connectors, and account_status. Extracted verbatim from the legacy god-module;
tool signatures and docstrings are unchanged.
...
def list_connectors() -> dict:
    """List available data-source connectors (e.g. 'stripe')."""
...
def connect_stripe(limit: int = 1000, stripe_key: str | None = None) -> dict:
    ...
    new analysis session, normalized to canonical tables.
...
def stripe() -> str:
    ...
    return get_connector("stripe").platform_prompt
