...
name: raise-pr
description: >
  Open a pull request for the current feature branch. Use when the user is ready
  to push the current feature branch and open a PR to master/main — runs
  pre-flight checks (confirms we're not on master/main, runs tests/lints),
