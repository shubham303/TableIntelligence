...
def _session(fixture: str, tmp_path) -> Session:
    ...
    shutil.copy(f"tests/fixtures/{fixture}", dst)
...
def test_combine_columns_ratio_and_is_feature(tmp_path):
...
def test_combine_columns_divide_by_zero_is_nan(tmp_path):
    s = _session("employees.csv", tmp_path)
    t = _table(s)
...
def test_combine_columns_rejects_unknown_op(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_transform_log_masks_nonpositive(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_transform_zscore_is_standardized(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_bin_column_quantile(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_group_aggregate_broadcasts_and_deviation(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_row_aggregate_sum(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_normalize_fractions_sum_to_one(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_row_aggregate_requires_two_columns(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
    with pytest.raises(ValueError):
...
def test_compute_feature_scalar_expression_is_a_feature(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_compute_feature_window_runs_in_db(tmp_path):
...
def test_compute_feature_replace_semantics(tmp_path):
    t = _table(_session("employees.csv", tmp_path))
...
def test_compute_feature_rejects_unsafe_expressions(tmp_path, expr):
    t = _table(_session("employees.csv", tmp_path))
    with pytest.raises(ValueError):
